...
fn get_line_values<'a>(ret: &mut Vec<String>, entry: &'a [u8]) -> IResult<&'a [u8], &'a [u8]> {
    match get_column_value(entry) {
        IResult::Done(in_, out) => {
            if in_.len() < 1 && out.len() < 1 { // this condition fails
                ret.push(String::new());
                IResult::Done(b"", b"")
            } else {
...
fn get_lines_values(ret: &mut Vec<Vec<String>>, entry: &[u8]) {
    match get_line(entry) {
        IResult::Done(in_, out) => {
            ...
            if ret.len() > 0 && ret[0].len() != line.len() {
                panic!("Line {} doesn't have the same length ({} instead of {})",
                    ret.len(), line.len(), ret[0].len())
            }
            ret.push(line);
            if in_.len() > 0 && in_[0] as char == '\n' {
                match take!(in_, 1) {
